...
function credit_record($username, $amount, $editor, $reason, $note = '', $ids = 0) {
	global $db, $DT_TIME, $DT;
	if($DT['log_credit'] && $username && $amount) {
		...
		$reason = addslashes(stripslashes(strip_tags($reason)));
		$note = addslashes(stripslashes(strip_tags($note)));
		$db->query("INSERT INTO {$db->pre}finance_credit (username,amount,balance,addtime,reason,note,editor,ids) VALUES ('$username','$amount','$balance','$DT_TIME','$reason','$note','$editor','$ids')");
	}
}
