...
function send_guestbook($areaid,$title,$content,$truename,$telephone,$username,$address) {
	global $db, $DT_TIME, $DT_IP;
	if(check_name($username) && $title && $content) {
		$title = addslashes($title);
		$content = addslashes($content);
		$db->query("INSERT INTO {$db->pre}guestbook (areaid,title,content,truename,telephone,username,ip,addtime,status,address) VALUES ('$areaid', '$title', '$content', '$truename','$telephone','$username','$DT_IP','$DT_TIME','3', '$address')");
		return true;
	}
	return false;
}
...
/**
  * cat_pass [根据当前分类ID调用上级ID]
  *
  * @param type $catid  [分类ID]
  * @param type $str    [字段]
  * @param type $parent [是否为上级]
  *
  * @return type
**/
function cat_pass($catid,$str,$parent=0) {
	global $db;
	if(!$catid) return '';	
	$data = $db->get_one("SELECT * FROM {$db->pre}category WHERE catid=$catid");
	
	if($parent){
		$parentdata = $db->get_one("SELECT * FROM {$db->pre}category WHERE catid={$data[parentid]}");
		$pos = $str?$parentdata[$str]:$parentdata;
	}else{
		$pos = $str?$data[$str]:$data;
	}
	return $pos;
}
...
function get_area($areaid) {
	...
	return $areaid ? $db->get_one("SELECT * FROM {$db->pre}area WHERE areaid=$areaid") : array();
}
...
function get_maincat($catid, $moduleid, $level = -1,$ceng=0) {
	global $db;
	$catid = intval($catid);
	...
	$cat = array();
	if($ceng){
		$result = $db->query("SELECT catid,catname,child,style,linkurl,item FROM {$db->pre}category WHERE $condition ORDER BY level,catid ASC", 'CACHE');
...
function friend_date($time)
{
    ...
    return iconv('GB2312', 'UTF-8', $fdate);
}
...
function get_qudao($var) {
	...
	foreach($option_value as $k=>$v){
		$qudaoList[] = explode("|",$v);
		//var_dump($qudaoList[$k][1]);
		$qd[(int)$qudaoList[$k][0]] = $qudaoList[$k][1];
	}
	return $qd[$var];
}
?>
