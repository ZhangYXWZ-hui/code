...
$option_value= $db->get_one("SELECT option_value FROM {$DT_PRE}fields WHERE itemid=66");
$option_value = array_filter(explode("*",$option_value["option_value"]));
...
if($submit) {
	...
	if(empty($post['email'])){
		$post['email'] = md5($post['username'].$post['password'].time())."@xiaobao.com";
	}
	$RG = array();
