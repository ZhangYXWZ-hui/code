	...
	function bind($username, $addtime) {
		...
		if($auth) {
			...
			if(strpos($auth, '|') !== false) {
				...
				if($itemid && $site) {
					$this->db->query("UPDATE {$this->db->pre}oauth SET username='' WHERE username='$username' AND site='$site'");
		...
		if($auth) {
			...
			if(is_openid($openid)) {
				$this->db->query("UPDATE {$this->db->pre}weixin_user SET username='' WHERE username='$username'");
