...
if($DT_QST) {
	...
	if($day&&$day==1){
		$condition .= " AND addtime > ".mktime(0, 0, 0, date("m"), date("d"), date("Y"));
	}elseif($day&&$day==2){
		$condition .= " AND addtime < ".(mktime(0, 0, 0, date('m'), date('d'), date('Y'))-1)." AND addtime > ".mktime(0, 0, 0, date("m"), date("d")-1, date("Y"));
	}else{
		if($day) $condition .= " AND addtime > ".(mktime(0, 0, 0, date("m"), date("d"), date("Y"))-(24*3600*$day));
	}
	...
	if($items) {
		if($addtime){
			$order = " ORDER BY addtime {$addtime}";
		}else{
