...
switch($action) {
	...
	case 'send':
		...
		$areaid = dhtmlspecialchars(trim($areaid));
		$address = dhtmlspecialchars(trim($address));
		...
		if($job == 'guestbook') {
			if(send_guestbook($areaid,$title,$content,$truename,$telephone,$username,$address)) {
				dalert($L['msg_home_success'], '', 'self.location=document.referrer;');
			} else {
