...
<td class="tab" id="action_invite"><a href="?action=invite"><span>推广赚<?php echo $DT['credit_name'];?></span></a></td>
<?php if($action == 'less') { ?>
...
<td><input type="radio" name="type" id="type_<?php echo $k;?>" value="<?php echo $k;?>" <?php if($k==$select) { ?>checked<?php } ?>
/></td>
<td class="f_red" id="c_<?php echo $k;?>"><?php echo $v;?></td>
<td class="f_blue" id="p_<?php echo $k;?>"><?php echo $P[$k];?></td>
</tr>
<?php } } ?>
</table>
</td>
...
function check() {
if($('#p_'+k).html() > <?php echo $_money;?>) {
Go('charge.php?action=pay&reason=credit|'+k+'&amount='+($('#p_'+k).html() - <?php echo $_money;?>));
return false;
}
var f,l;
f = 'password';
...
window.setInterval(
function() {
$('#p_'+k).html() > <?php echo $_money;?> ? Dh('payword') : Ds('payword');
}, 
500);
</script>
